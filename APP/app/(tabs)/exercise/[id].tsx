  ...
  useEffect(() => {
    ...
    return () => {
      if (soundRef.current) {
        soundRef.current
          .unloadAsync()
          .catch((err) => console.warn("Erreur unload sound", err));
      }
      if (intervalRef.current) {
        clearInterval(intervalRef.current);
      }
    };
  }, [id, user?.userId, loadExercise]);

  const toggleFavorite = async () => {
    try {
      if (!user?.userId || !id || typeof id !== "string") return;

      if (isFavorite) {
        const res = await axios.get(`${API_URL}/favorites/user/${user.userId}`);
